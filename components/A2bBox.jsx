      ...
      <div className="p-8">
        <div>
          ...
          {/* <div className="grid grid-cols-12 gap-5 p-8 text-center">
            {flags.map((flag) => (
              flag !== langcodes[0]?.toLowerCase() && flag !== langcodes[1]?.toLowerCase() && (flag !== 'english' || langcodes[0] + langcodes[1] !== '') && (
                <Link
                  key={flag}
                  href={langcodes[0] ? `/translate-${langcodes[0].toLowerCase()}-${vord}-to-${flag}/` : langcodes[1] ? `/translate-${flag}-${vord}-to-${langcodes[1].toLowerCase()}/` : `/translate-${flag}-${vord}-to-english/`}
                >
                  <a className="col-span-6 flex flex-col gap-1 rounded-xl border-2 border-gray-600/30 py-2 px-1 sm:col-span-4 md:col-span-3">
                    <div className="capitalize text-sm sm:text-base">{flag}</div>
                    <div className="relative">
                      <span style={{ boxSizing: 'border-box', display: 'inline-block', overflow: 'hidden', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: 0, margin: 0, padding: 0, position: 'relative', maxWidth: '100%' }}>
                        <span style={{ boxSizing: 'border-box', display: 'block', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: 0, margin: 0, padding: 0, maxWidth: '100%' }}>
                          <img style={{ display: 'block', maxWidth: '100%', width: 'initial', height: 'initial', background: 'none', opacity: 1, border: 0, margin: 0, padding: 0 }} alt="" aria-hidden="true" src="data:image/svg+xml,%3csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20version=%271.1%27%20width=%2750%27%20height=%2733%27/%3e" />
                        </span>
                        <img
                          alt={`Image of ${flag} flag`}
                          src={`/static/svgs/${flag}.svg`}
                          decoding="async"
